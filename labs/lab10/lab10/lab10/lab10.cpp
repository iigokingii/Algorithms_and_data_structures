...
void main()
{
	...
	Way way = antAlgorithm(cities, pheromons, N, nIters, 2, 2, 0);
	cout << "лучший путь : ";
	cout << way.to_string();
}
	...
	size_t current)
{
	...
	for (size_t i = 0; i < propabs.size(); i++)
	{
		if (notv[i])
		{
			propabs[i] = 100.0 * pow(4 / (double)graph[current][i], alpha) * pow((double)pheramons[current][i], beta) / sum;
		}
		else
		{
			propabs[i] = 0;
